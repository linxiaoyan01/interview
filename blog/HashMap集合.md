...
![](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094444.png)
...
![](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094440.png)
...
![](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094434.png)
...
![](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094430.bmp)
...
​    2.另一方面，一般我们可能会想通过 % 求余来确定位置，这样也可以，只不过性能不如 & 运算。而且当length是2的幂次方时：hash & (length - 1) == hash % length
...
= x*2^(n-1-m) + x*2^(n-2-m) +... + x*2^(m-m) + x*2^(m-1-m) + ... + x*2^(0-m)
...
public HashMap(int initialCapacity, float loadFactor) {//initialCapacity=10
  ...
  */
    static final int tableSizeFor(int cap) {//int cap = 10
        int n = cap - 1;
        n |= n >>> 1;//假如最高位1移动一位再或运算，变成11
        n |= n >>> 2;//11移动两位再或运算，变成1111
        n |= n >>> 4;//1111移动四位再或运算，变成11111111
        n |= n >>> 8;//11111111移动八位再或运算，变成1111111111111111
        n |= n >>> 16;//1111111111111111移动十六位再或运算，变成11111111111111111111111111111111
...
![image-20210615193541433](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615193553.png)
...
以此类推
注意，容量最大也就是32bit的正数，因此最后n |= n >>> 16; ，最多也就32个1（但是这已经是负数了。在执行tableSizeFor之前，对initialCapacity做了判断，如果大于MAXIMUM_CAPACITY(2 ^ 30，即只有第31位为1，经过上述操作会使得第31位后面的所有位都为1)，则取MAXIMUM_CAPACITY。如果等于MAXIMUM_CAPACITY(2 ^ 30)，先减一，再执行移位操作。所以这里面的移位操作之后，最大30个1，不会大于等于MAXIMUM_CAPACITY。30个1，加1之后得2 ^ 30） 。
请看下面的一个完整例子：

![image-20191115151657917](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094422.png)
...
![image-20191115161055901](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094418.png)
...
![image-20191115173553375](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094413.png)
...
![image-20191114193730911](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094407.png)
...
![image-20191117110812839](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094353.png)

 因此元素在重新计算hash之后，因为n变为2倍，那么n-1的标记范围在高位多1bit(红色)，因此新的index就会发生这样的变化： 

![image-20191117110934974](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094350.png)
...
![image-20191117111211630](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094346.png)
...
我们如何判断`hash`值多出来的那一位是`0`还是`1`呢，很简单，只要用`hash`值与旧容量做与运算，结果不为`0`表示多出的这一位是`1`，否则就是0。比如说，容量为`8`（二进制`1000`），扩容后多出来的是第四位，于是让`hash`值与`1000`做与运算，若`hash`值的第四位是`1`，与`1000`做与运算后结果就是`1000`，若第四位是`0`，与`1000`做与运算后就是`0`。
...
```java
    ...
    final void split(HashMap<K,V> map, Node<K,V>[] tab, int index, int bit) {
        ...
        if (loHead != null) {
            ...
            //然后让这个桶中，要还原索引位置开始往后的结点都还原成链表的 lXXX 节点
            //这一段元素以后就是一个链表结构
...
#### 红黑树删除方法(TreeNode.removeTreeNode)
 ...
 final TreeNode<K,V> find(int h, Object k, Class<?> kc) {
            ...
            do {
                ...
                if ((ph = p.hash) > h)
                    ...
                    //反之在p的右子树中继续查找
                    p = pr;
                 //若两节点hash值相等，且节点的key也相等，则匹配成功，返回p
                else if ((pk = p.key) == k || (k != null && k.equals(pk)))
                    return p;
                
                /***---- 下面的情况是节点p的hash值和h相等，但key不匹配，需继续在p的子树中寻找 ----****/
                //若p的左子树为空，则直接在右子树寻找。若右子树也为空，则会不满足循环条件，返回null，即未找到
                else if (pl == null)
                    p = pr;
                //反之若左子树不为空，同时右子树为空，则继续在左子树中寻找
                else if (pr == null)
                    p = pl;
...
![image-20210615105022071](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615105024.png)

2、使用Iterator迭代器迭代 

![image-20210615105045275](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615105047.png)

3、通过get方式（不建议使用）

![image-20210615105107212](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615105109.png)
...
![image-20191117164748836](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094335.bmp)
...
![image-20191117165438726](https://gitee.com/YuerryHUAHUA/figure/raw/master/img/20210615094328.png)
...
HashMap 的设计目标是简洁高效，没有采取任何措施保证 put、remove 操作的多线程安全。 HashMap 的 put 方法的操作对象要么是整个散列表，要么是某个哈希桶里的链表或红黑树，而这些过程都没有采取措施保证多线程安全。在这个复杂的逻辑过程中，任何一个线程在这个过程中改动了散列表的结构，都有可能造成另一个线程的操作失败。

Java8 中的 HashMap 多线程情况下可能出错的一个例子。1.8虽然没有了死链的问题，但还是存在size()问题，++size不是原子操作。
...
ConcurrentHashMap 与 HashMap 相比，在高效率的基础上添加了对多线程安全的保证。为了这个多线程安全，ConcurrentHashMap 并没有采用 HashMap 的单散列表设计，而是引入了分段散列表来平衡多线程安全和高效率，核心思想就是尽量只锁住需要操作的散列表段，而不是整个散列表。

HashTable 与 ConcurentHashMap 相比，对并发的处理粗糙了许多，单纯地通过锁住整个散列表来保证多线程安全性。虽然在设计和实现上比 ConcurrentHashMap 简单了许多，但在效率上远远不及 ConcurrentHashMap，因此目前多线程情况下不建议使用 HashTable，只推荐使用 ConcurrentHashMap。
